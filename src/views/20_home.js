...
<span class="dm">&gt;</span> status: <span class="gs">READY</span></div>
  ...
  <p class="vlede">Нельзя ломать то, чего не понимаешь. Прежде чем учить XSS, SQLi и SSRF, надо знать, как вообще устроен веб: что происходит между нажатием Enter в адресной строке и появлением пикселей на экране. Здесь этот путь разобран по шагам — от физики сети до модели безопасности браузера. Начинаешь с нуля — читай подряд. Веб знаешь давно, но детали стёрлись — бери секции точечно и проверяй себя квизами. Ничего не выдумано: каждый факт сверен с MDN, RFC, OWASP, PortSwigger и web.dev, ссылки лежат прямо в модулях.</p>
